# cachetag
Founder's note:
I solved a problem I was seeing everywhere - poor AI recall affecting productivity, multiple queries, and rising costs and wasted planet resources. People saying "Add more compute!"..while cutting precious jobs. "Add more compute!" while not finding efficiencies. So I focused for an hour or two and invented a solution first for myself, then tested it for larger data sets. I architected #cachetag as a stateless context protocol rather than a stateful database layer. 
...
Because it is stateless, it injects zero memory or lookup overhead, relying instead on Anthropic’s prefix-matching cache topology to guarantee a 90% drop in token compute costs.
